...
const currenType = ref<RouteQuery["type"]>();
const currenCategory = ref<string>("");

const mouseEnterItem = (type: string, index: number) => {
...
onMounted(() => {
  ...
  if (type && category) {
    ...
    if (type === "front" && category in frontCategory) {
      currenCategory.value = (frontCategory as Record<string, CategoryItem>)[category].name;
      siteList.value = (frontSite as Record<string, SiteItem[]>)[category];
    } else if (type === "back" && category in backCategory) {
      currenCategory.value = (backCategory as Record<string, CategoryItem>)[category].name;
