...
const currenCategory = ref<CategoryMap>({});
  ...
  <div class="center_box">
    <el-row class="category">
      <el-col
        :xs="12"
        :sm="6"
        v-for="(item, index) in Object.keys(currenCategory)"
        :key="index"
      >
        <el-card class="card-hover">
          ...
          >
            ...
            >
              ...
              <el-avatar :size="60">
                <span>{{ currenCategory[item].name }}</span>
              </el-avatar>
              <span :class="['bottom', activeType && active == index ? 'bottom-animate' : '']"></span>
            </div>
            <div class="right">
              <span
                :title="currenCategory[item].name"
                class="name"
              >
                {{ currenCategory[item].name }}
              </span>
              <span
                :title="currenCategory[item].desc"
                class="desc"
              >
                {{ currenCategory[item].desc }}
              </span>
            </div>
          </div>
        </el-card>
